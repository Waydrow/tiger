...
public class Main {
    ...
    private Slp.IntAndTable interpExp(Exp.T exp, Slp.Table t) {
        if (exp instanceof Exp.Id) {
            return new Slp.IntAndTable(t.lookup(t, ((Id) exp).id), t);
        } else if (exp instanceof Exp.Num) {
            return new Slp.IntAndTable(((Num) exp).num, t);
        } else if (exp instanceof Exp.Op) {
            Slp.IntAndTable s1 = interpExp(((Op) exp).left, t);
            Slp.IntAndTable s2 = interpExp(((Op) exp).right, s1.t);
            switch (((Op) exp).op) {
                case ADD:
                    return new Slp.IntAndTable(s1.i + s2.i, s2.t);
                case SUB:
                    return new Slp.IntAndTable(s1.i - s2.i, s2.t);
                case TIMES:
                    return new Slp.IntAndTable(s1.i * s2.i, s2.t);
            ...
            Slp.Table ss = interpStm(((Eseq) exp).stm, t);
            return interpExp(((Eseq) exp).exp, ss);
        }
        return null;
    }
